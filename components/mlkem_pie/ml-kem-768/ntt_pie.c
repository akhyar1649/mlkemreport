...
const int16_t __attribute__((aligned(16))) PQCLEAN_MLKEM768_CLEAN_zetas[128] = {
        ...
        for (start = 0; start < 256; start = j + len) {
            ...
            if (len >= 8) {
                ...
                for (j = start; j + 15 < start + len; j += 16) {
                    ...
                    for (i = 0; i < 8; i++) {
                        r[j + i] = PQCLEAN_MLKEM768_CLEAN_barrett_reduce(sumvec0[i]);
                        r[j + 8 + i] = PQCLEAN_MLKEM768_CLEAN_barrett_reduce(sumvec1[i]);
                    }
